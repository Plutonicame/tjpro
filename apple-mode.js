...
html.tjp-apple-mode {
  ...
  color-scheme: dark; /* évite que les contrôles natifs (select, scrollbar...) restent clairs */
}
...
/* ── Filet de sécurité — l'app a des dizaines de variables --crt-* posées
   au cas par cas (data-tvar) sur des éléments qu'il est impossible de
   tous cataloguer un par un. Sans ce filet, un élément non repris plus
   bas garde son ancienne couleur perso (ex. texte sombre pensé pour un
   fond clair d'origine) posée sur nos nouveaux fonds sombres → zones
   invisibles ou incohérentes. Les règles plus bas, plus spécifiques,
   reprennent la main normalement (une classe bat toujours *). ── */
html.tjp-apple-mode * {
...
`;
  ...
  // Couleurs des graphiques (lues via gc('--xxx') par drawEquity/drawPnl/
  // etc.) : jamais touchées par une simple classe CSS puisque Chart.js les
  // lit en JS. On les pose directement comme variables inline (même
  // mécanisme que previewTheme()) quand le mode est activé, et on les
  // restaure via previewTheme() (qui réapplique le vrai thème du compte
  // et redessine) quand il est désactivé.
  var AM_CHART_VARS = {
    '--eq-line': '#0A84FF', '--eq-axis': 'rgba(235,235,245,0.6)', '--eq-grid': 'rgba(255,255,255,0.08)',
    '--eq-fill-top': 'rgba(10,132,255,0.35)', '--eq-fill-bot': 'rgba(10,132,255,0)',
    '--pnl-axis': 'rgba(235,235,245,0.6)', '--pnl-grid': 'rgba(255,255,255,0.08)',
    '--pnl-bar-pos': '#30D158', '--pnl-bar-neg': '#FF453A',
    '--pnl-avg-gain': '#30D158', '--pnl-avg-loss': '#FF453A',
    '--risk-axis': 'rgba(235,235,245,0.6)', '--risk-grid': 'rgba(255,255,255,0.08)',
    '--risk-line': '#0A84FF', '--risk-base': 'rgba(235,235,245,0.35)',
    '--risk-fill-top': 'rgba(10,132,255,0.3)', '--risk-fill-bot': 'rgba(10,132,255,0)',
    '--pie-win': '#30D158', '--pie-lose': '#FF453A', '--pie-neutral': 'rgba(235,235,245,0.35)',
    '--winrate-slice-border': '#000000',
    '--comp-axis': 'rgba(235,235,245,0.6)', '--comp-grid': 'rgba(255,255,255,0.08)',
    '--comp-pos': 'rgba(48,209,88,0.85)', '--comp-neg': 'rgba(255,69,58,0.85)',
    '--mgmt-axis': 'rgba(235,235,245,0.6)', '--mgmt-grid': 'rgba(255,255,255,0.08)',
    '--badge-p-pos-tx': '#30D158', '--badge-p-neg-tx': '#FF453A',
    '--kpi-strip-background': 'rgba(255,255,255,0.08)'
  };

  // N'agit que sur un vrai changement d'état — sinon loadSavedTheme(),
  // appelé très souvent (connexion, changement de sous-compte...), aurait
  // redessiné tous les graphiques à chaque fois même quand le mode Apple
  // n'est pas concerné, ce qui ralentissait/perturbait le journal.
  function amApply(on) {
